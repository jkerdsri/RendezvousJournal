...
It can be obviously seen that the proposed protocols in Fig. \ref{Protocol Performance per Node Density} can save about half of energy consumption in order to achieve the same delivery ratio in the dense environment.
...
Additionally, the other factor that impact the higher protocol performance is the lower average wireless transmission of our proposed protocol.
...
% \caption{protocol Performance per Power Saving Factor}
% \label{protocol Performance per Power Saving Factor}
% \end{figure}
...
%%
On the other hand, the protocol performance is increasing with the value of power saving factor which shows that the higher the $n_{ps}$, the higher the protocol performance.
%%
Overall, the key point of Fig. \ref{The Optimum between Delivery Ratio and protocol Performance} is the cross point between delivery ratio and protocol performance which can see at $n_{ps} = 0.55$ which is the optimum point of our method.
...
% %%
% However, the protocol performance is increasing with the power saving factor as seen in Fig. \ref{protocol performance per Power Saving Factor}.
% %%
% This result illustrates that in order to gain higher protocol performance, the power saving factor is also increasing.



% As a result, the trade-off between the delivery ratio and protocol performance is shown in Fig. \ref{The Optimum between Delivery Ratio and protocol performance}.
% From the graph, the optimum is on the power saving factor of 0.6




% In this section, we study the factors effecting the power saving of the proposed protocols by analyzing the impact of $\tau_s$ on the protocol performance as seen in Fig. \ref{Utilization ratio of wake-up interval comparison}.
...
% The result from Fig. \ref{Utilization ratio of wake-up interval comparison} shows that the value of $\tau_s$ varies directly to the protocol performance, which suggests that keeping the state of  operation in saving mode longer resulting in higher $D_p$.
...
% Therefore, to increase the protocol performance, we have to escalate the value of $\tau_s$ and $R_s$ which means maintaining the operation of power saving mode as long as possible while extending the ${ r }_{ c }^{ min }$.
% The reason behind this result comes from Eq. \ref{eq:protocol_performance}, in which the value of radio range is the leading factor effecting the protocol performance.
% In parallel, the time period of ${ r }_{ c }^{ min }$ state is effecting the value of $D_p$ since the result shows that the protocol performance varies directly to the $\tau_s$.
...
%Fig. \ref{Utilization ratio of wake-up interval comparison} shows the protocol performance of each node density varying by $\tau_{s}$.
%The result clearly shows that the protocol performance increase with the value of $\tau_s$. 
